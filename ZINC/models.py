...
class VAEEncoder(nn.Module):
    ...
    def __init__(self, in_dimension, latent_dimension):
        ...
        self.encode_nn = nn.Sequential(
            nn.Conv1d(in_dimension, 9, kernel_size=9),
            nn.ReLU(),
            nn.Conv1d(9, 9, kernel_size=9),
            nn.ReLU(),
            nn.Conv1d(9, 11, kernel_size=10),
            nn.ReLU()
        )

        self.flatten = nn.Flatten()
        self.linear_1 = nn.Linear(121, 435)

        # Latent space mean
        self.encode_mu = nn.Linear(435, latent_dimension)
        # Latent space variance
        self.encode_log_var = nn.Linear(435, latent_dimension)
